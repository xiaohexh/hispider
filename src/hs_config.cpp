...
HSConfig::HSConfig() : m_iLoaded(0), m_nullRet("")
{	
}
...
HSConfig * HSConfig::instance()
{
	static HSConfig * _ins  = 0;
...
int HSConfig::loadConfigFile(const  char * s_cnf_file)
{
	if(NULL == s_cnf_file) {
...
*************************************************/
int HSConfig::getConfigInt(const string& s_section, const string& s_item)
{
...
@Others:        svr运行时候获取，如果不能获取，证明配置有误
*************************************************/
const char * HSConfig::getConfigStr(const string& s_section, const string& s_item)
{
   CONFIG_CONTENER::iterator iter = m_contener.find(s_section);
...
const string & HSConfig::getCfgStr(const string& s_section, const string& s_item)
{
   CONFIG_CONTENER::iterator iter = m_contener.find(s_section);
...
//@Others:        svr运行时候获取，如果不能获取，证明配置有误
//*************************************************/
//string HSConfig::loadConfigStr(const string& s_section, const string& s_item)
//{
//    int iRet;
//    char sResult[1024];
